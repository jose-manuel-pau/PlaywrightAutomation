...
test('Eligible for Refund', async ({ page }) => {
    ...
    await navigateBookingDetails(page, 1);
...
test('Not Eligible Refund', async ({ page }) => {
    ...
    await navigateBookingDetails(page, 3);
...
async function navigateBookingDetails(page, ticketQuantity) {
    ...
    const ticketText = ticketQuantity === 1 ? '1 ticket' : `${ticketQuantity} tickets`;

    const bookingCard = page
        .locator('#booking-card')
        .filter({ hasText: ticketText })
        .first();
